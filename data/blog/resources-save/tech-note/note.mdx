...
| 取消订阅方式                  | 特点与区别                                                                               | 适用场景                                                               |
| ----------------------------- | ---------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| 通过订阅令牌（token）取消订阅 | - 精确性：只取消与该令牌关联的特定订阅。- 作用范围：仅影响单个订阅。                     | 需要取消特定订阅而不影响其他订阅的场景。                               |
| 通过主题名称取消订阅          | - 广泛性：取消该主题下的所有订阅者。- 作用范围：影响所有在该主题上注册的回调函数。       | 需要清理某个主题的所有订阅者的场景，如应用程序关闭或模块卸载时。       |
| 通过订阅函数取消订阅          | - 灵活性：取消所有使用该函数作为回调的订阅。- 作用范围：影响所有使用相同回调函数的订阅。 | 需要取消某个特定函数的所有订阅的场景，如某模块不再需要处理任何主题时。 |
